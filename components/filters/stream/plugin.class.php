...
class plugin_stream extends plugin_base{
	...
	private function execute_sql($finalelements, $data) {
		$filter_stream = optional_param('filter_stream_'.$data->field,0,PARAM_RAW);
		$filter = clean_param(base64_decode($filter_stream),PARAM_CLEAN);

		if($filter_stream &&
		   preg_match("/%%FILTER_STREAM:([^%]+)%%/i",$finalelements, $output)){
			$replace = ' AND '.$output[1].' = '. "'$filter'";
			return str_replace('%%FILTER_STREAM:'.$output[1].'%%',$replace,$finalelements);
		}
	...
	function print_filter(&$mform, $data){
		...
		$mform->addElement('select', 'filter_stream_' . $data->field, $selectname, $filteroptions);
		$mform->setType('filter_stream_' . $data->field, PARAM_TEXT);
	}	
}
