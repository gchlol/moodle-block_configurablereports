 ...
 */
class plugin_stream extends plugin_base {
    ...
    public function init(): void {
        $this->form = true;
        $this->unique = true;
        $this->fullname = get_string('stream', 'block_configurable_reports');
    ...
    private function execute_sql($finalelements, $data) {
        $filterstream = optional_param('filter_stream_' . $data->field, 0, PARAM_BASE64);
        $filter = base64_decode($filterstream);
        $filtermatch = preg_match("/%%FILTER_STREAM:([^%]+)%%/i", $finalelements, $output);

        if ($filterstream && $filtermatch) {
            $replace = ' AND ' . $output[1] . ' = ' . "'$filter'";
            $finalelements = str_replace('%%FILTER_STREAM:' . $output[1] . '%%', $replace, $finalelements);
        }
    ...
    public function print_filter(MoodleQuickForm $mform, $formdata = false): void {
        ...
        $mform->addElement('select', 'filter_stream_' . $formdata->field, $selectname, $filteroptions);
        $mform->setType('filter_stream_' . $formdata->field, PARAM_TEXT);
    }
}
