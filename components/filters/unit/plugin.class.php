...
class plugin_unit extends plugin_base{
	...
	private function execute_sql($finalelements, $data) {
		$filter_unit = optional_param('filter_unit_'.$data->field,0,PARAM_RAW);
		$filter = clean_param(base64_decode($filter_unit),PARAM_CLEAN);

		if($filter_unit &&
		   preg_match("/%%FILTER_UNIT:([^%]+)%%/i",$finalelements, $output)){
			$replace = ' AND '.$output[1].' = '. "'$filter'";
			return str_replace('%%FILTER_UNIT:'.$output[1].'%%',$replace,$finalelements);
		}
	...
	function print_filter(&$mform, $data){
		...
		$mform->addElement('select', 'filter_unit_' . $data->field, $selectname, $filteroptions);
		$mform->setType('filter_unit_' . $data->field, PARAM_TEXT);
