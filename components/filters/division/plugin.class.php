...
class plugin_division extends plugin_base {
    ...
    private function execute_sql($finalelements, $data) {
        $filterdivision = optional_param('filter_division_' . $data->field, 0, PARAM_BASE64);
        $filter = base64_decode($filterdivision);
        $filtermatch = preg_match("/%%FILTER_DIVISION:([^%]+)%%/i", $finalelements, $output);

        if ($filterdivision && $filtermatch) {
            $replace = ' AND ' . $output[1] . ' = ' . "'$filter'";
            $finalelements = str_replace('%%FILTER_DIVISION:' . $output[1] . '%%', $replace, $finalelements);
        }
    ...
    public function print_filter(MoodleQuickForm $mform, $formdata = false): void {
        ...
        $mform->addElement('select', 'filter_division_' . $formdata->field, $selectname, $filteroptions);
        $mform->setType('filter_division_' . $formdata->field, PARAM_TEXT);
    }
}
