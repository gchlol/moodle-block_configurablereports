...
class plugin_division extends plugin_base{
	
	function init(){
		$this->form = true;
		$this->unique = true;
		$this->fullname = get_string('division','block_configurable_reports');
	...
	private function execute_sql($finalelements, $data) {
		$filter_division = optional_param('filter_division_'.$data->field,0,PARAM_RAW);
		$filter = clean_param(base64_decode($filter_division),PARAM_CLEAN);

		if($filter_division &&
		   preg_match("/%%FILTER_DIVISION:([^%]+)%%/i",$finalelements, $output)){
			$replace = ' AND '.$output[1].' = '. "'$filter'";
			return str_replace('%%FILTER_DIVISION:'.$output[1].'%%',$replace,$finalelements);
		}
	...
	function print_filter(&$mform, $data){
		...
		if(!empty($userlist)){
			if(strpos($data->field,'profile_') === 0){	
				...
				list($usql, $params) = $DB->get_in_or_equal($userlist);
				$sql = "SELECT DISTINCT(".$data->field.") as ufield FROM {user} WHERE id $usql AND u.suspended = 0 AND u.deleted = 0 ORDER BY ufield ASC";
		...
		$mform->addElement('select', 'filter_division_' . $data->field, $selectname, $filteroptions);
		$mform->setType('filter_division_' . $data->field, PARAM_TEXT);
	}	
}
