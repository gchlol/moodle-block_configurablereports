...
class plugin_department extends plugin_base{
	...
	private function execute_sql($finalelements, $data) {
		...
		if($filter_department &&
		   preg_match("/%%FILTER_DEPARTMENT:([^%]+)%%/i",$finalelements, $output)){
	...
	function print_filter(&$mform, $data){
		...
		$mform->addElement('select', 'filter_department_' . $data->field, $selectname, $filteroptions);
		$mform->setType('filter_department_' . $data->field, PARAM_TEXT);
