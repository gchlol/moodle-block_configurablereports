...
class plugin_department extends plugin_base {
    ...
    private function execute_sql($finalelements, $data) {
        $filterdepartment = optional_param('filter_department_' . $data->field, 0, PARAM_BASE64);
        $filter = base64_decode($filterdepartment);
        $filtermatch = preg_match("/%%FILTER_DEPARTMENT:([^%]+)%%/i", $finalelements, $output);

        if ($filterdepartment && $filtermatch) {
            $replace = ' AND ' . $output[1] . ' = ' . "'$filter'";
            $finalelements = str_replace('%%FILTER_DEPARTMENT:' . $output[1] . '%%', $replace, $finalelements);
        }
    ...
    public function print_filter(MoodleQuickForm $mform, $formdata = false): void {
        ...
        $mform->addElement('select', 'filter_department_' . $formdata->field, $selectname, $filteroptions);
        $mform->setType('filter_department_' . $formdata->field, PARAM_TEXT);
    }
}
